...
const Calendar = React.memo(function Calendar({
  ...
  const memoizedComponents = React.useMemo(
    () => ({
      Root: ({ className: rootClassName, rootRef, ...rootProps }: React.ComponentProps<"div"> & { rootRef?: React.Ref<HTMLDivElement> }) => (
        <div
          data-slot="calendar"
          ref={rootRef}
      ...
      Chevron: ({ className: chevronClassName, orientation, ...chevronProps }: { className?: string; orientation?: "left" | "right" | "up" | "down" }) => {
        if (orientation === "left") {
          return (
      ...
      WeekNumber: ({ children, ...weekNumProps }: React.ComponentProps<"td">) => (
        <td {...weekNumProps}>
...
const CalendarDayButton = React.memo(function CalendarDayButton({
  ...
  const isSingleSelected = React.useMemo(
    () =>
      Boolean(
        modifiers?.selected &&
  ...
  const formattedDate = React.useMemo(() => {
    try {
      return day?.date instanceof Date && !isNaN(day.date.getTime())
